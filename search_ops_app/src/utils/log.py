...
class Logger:
    ...
    def __init__(self, logger_name: str = __name__):
        ...
        Args:
            ...
            name of logger. Defaults to __name__.
        """
    ...
    def get_logger(
        ...
        Args:
            add_file_handler (str, optional):
            filename to store logging info. Defaults to "".
            add_stdout_handler (bool, optional):
            stdout handler identifier. Defaults to True.
            remove_exist_handler (bool, optional): whether to remove
            all the existing handlers. Defaults to False.
        ...
        if (
            ...
            or "StreamHandler" not in exist_handlers
        ) and add_stdout_handler:
            self.logger.addHandler(self._get_console_handler())

        if (
            not self.logger.hasHandlers()
            or "FileHandler" not in exist_handlers
        ) and add_file_handler:
            self.logger.addHandler(
                self._get_file_handler(add_file_handler)
            )
