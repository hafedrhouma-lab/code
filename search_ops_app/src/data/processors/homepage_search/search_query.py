"""Class to process Search Country queries"""
...
class SearchQuery:
    ...
    def process_data(self) -> None:
        ...
        initial_data_length = self.data_frame.shape[0]

        self.data_frame = self.data_frame[
            self.data_frame.number_query_sessions_that_placed_order <=
            self.data_frame.number_sessions_that_clicked
            ].reset_index(drop=True)

        _logger.info(
            f"Removed "
            f"{initial_data_length - self.data_frame.shape[0]} rows"
        )
    ...
    def compute_metrics(self) -> None:
        ...
        self.data_frame["session_CVR(%)"] = \
            np.round(
                self.data_frame[
                    "number_sessions_that_placed_order"
                ] / self.data_frame[
                    "number_sessions"
                ] * 100
            )
