...
class ChainFeaturesProcessor(BaseProcessor):
    ...
    def process(self):
        ...
        if 'event_timestamp' in chain_features.columns:
            chain_features = chain_features.sort_values(
                by=['chain_id', 'event_timestamp'],
        ...
        unique_sorted_df = chain_features.drop(columns=['country_code', 'event_timestamp'])

        chain_features = self.fill_missing_chain_features(unique_sorted_df)
    ...
    def fill_missing_chain_features(self, chains_df, logger=None):
        ...
        hour_pct_sums = chains_df[hour_pct_cols].sum(axis=1)
        mask = np.isclose(hour_pct_sums, 1)
        chains_df = chains_df[mask].reset_index(drop=True)
