...
import numpy as np
from .transformer_encoder_layer import GlobalSelfAttention
...
def create_vectorized_attention_layer(vocab, aggregation,
    ...
    vectorizer = tf.keras.layers.TextVectorization(
        vocabulary=list(vocab), standardize="strip_punctuation",
...
class chainModel_v2(tf.keras.Model):
    def __init__(
            self,
            chain_emb_size,
            cuisine_emb_size,
        ...
        if shared_chain_embedding:
            ...
            embedding = tf.keras.layers.Embedding(
                len(unique_chain_ids) + 1, chain_emb_size, name="chain_embedding"
            )
        self.chain_embedding = tf.keras.Sequential(
            [
                tf.keras.layers.StringLookup(
                    vocabulary=unique_chain_ids, mask_token="0", name="chain_lookup"
                ),
                embedding,
            ]
        )
        ...
        if 'chain_freq_area_ids' in candidates_features:
            self.freq_areas_tower = create_vectorized_attention_layer(unique_area_ids, aggregation='AVG',
                                                                      emebdding_size=chain_emb_size // 2,
                                                                      attention_dim=chain_emb_size // 2,
                                                                      layer_name_prefix='chain_freq_area_ids',
                                                                      predefined_embedding=shared_area_embedding,
                                                                      apply_attention=False)

            self.feat_to_partial_tower['chain_freq_area_ids'] = self.freq_areas_tower
        ...
        if monthly_orders is not None:
            ...
            self.feat_to_partial_tower["monthly_orders"] = lambda x: tf.reshape(
                self.monthly_order(x), (-1, 1)
            )
        ...
        if "chain_name" in candidates_features:
            ...
            if shared_search_embedding:
                ...
                chain_name_embedding = tf.keras.layers.Embedding(
                    len(unique_chain_ids) + 1, chain_emb_size
                )
        ...
        if rating is not None:
            self.rating = tf.keras.layers.Normalization(
                axis=None, mean=rating.mean(), variance=rating.var()
            )

            self.feat_to_partial_tower["rating"] = lambda x: tf.reshape(
                self.rating(x), (-1, 1)
            )
        ...
        if chain_gmv is not None:
            ...
            self.feat_to_partial_tower["chain_avg_gmv"] = lambda x: tf.reshape(
                self.chain_avg_gmv(x), (-1, 1)
            )
            # self.feat_to_partial_tower['rating'] = self.rating
    ...
    def call(self, chains_df):
        ...
        if len(self.feat_to_partial_tower) > 1:
            ...
            # print([(feature_col_dict[x], x) for x in available_features])
            # print(self.feat_to_partial_tower)
            output = tf.concat(
                [
...
# userModel_v2 seems outdated
class userModel_v3(tf.keras.Model):
    def __init__(
            self,
            user_emb_size,
            chain_emb_size,
        ...
        for feature in ['prev_clicks', 'freq_clicks', 'user_prev_chains', 'freq_chains']:
            if feature in query_features:
                feature_tower = create_vectorized_attention_layer(
                    ...
                    emebdding_size=chain_emb_size, attention_dim=chain_emb_size,
...
class AccountChainModel(tfrs.Model):
    def __init__(
        ...
        self.task = tfrs.tasks.Retrieval(
            metrics=tfrs.metrics.FactorizedTopK(
                candidates=candidates_ds.batch(128).map(self.chain_model),
                ks=(1,)
            ),
    ...
    def compute_loss(self, features, training=False):
        ...
        customer_embeddings = self.customer_model(
            {a: features[a+'_pos'] for a in self.query_features}, training=True
        )
        chain_embeddings = self.chain_model(
            {
                a: tf.concat([features[a+'_pos'], features[a+'_neg']], axis=0)
                for a in self.candidate_features
            },
            training=True,
        )
        candidate_probability = tf.concat(
            [features["candidate_probability_pos"], features["candidate_probability_neg"]], axis=0
        )
        candidate_ids = tf.concat(
            [features[self.unique_candidate+'_pos'], features[self.unique_candidate+'_neg']], axis=0
        )
        sample_weight = features["sample_weight_pos"]
        ...
        if self.unique_candidate_features is not None:
            ...
            chain_embeddings = tf.concat(
                [chain_embeddings, negatives_embeddings], axis=0
            )
...
def create_two_tower_model(
        ...
        enable_shared_area_ids_embedding=False,
        unique_order_sources=None
):
    ...
    if enable_shared_area_ids_embedding:
        shared_area_embedding = tf.keras.layers.Embedding(
            len(unique_area_ids) + 2,
            embedding_dimension,
    ...
    customer_model = tf.keras.Sequential(
        [
            userModel_v3(
                ...
                unique_order_sources=unique_order_sources
            ),
