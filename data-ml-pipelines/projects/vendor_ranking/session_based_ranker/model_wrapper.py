...
class ModelWrapper(MlflowBase):
    ...
    def load_context(self, context: mlflow.pyfunc.PythonModelContext) -> None:
        with perf_manager(
            ...
            self.offline_model.set_model_device(self.device)
            ...
            self.recommender.load_model_to_device(self.device)
    ...
    def preprocess_input(model_input, feature_configs, area_id_to_index, geohash_to_index, numerical_feature_names):
        ...
        model_input['delivery_area_id'] = torch.tensor([area_id_to_index.get(model_input['delivery_area_id'], area_id_to_index['unk'])], dtype=torch.long)
        model_input['geohash6'] = torch.tensor([geohash_to_index.get(model_input['geohash6'], geohash_to_index['unk'])], dtype=torch.long)
        model_input['numerical_features'] = torch.tensor([model_input[feat] for feat in numerical_feature_names], dtype=torch.float).T.unsqueeze(0)
        return model_input
    ...
    def predict(self, context: mlflow.pyfunc.PythonModelContext, request: dict[str, list[Any]]):
        ...
        newrelic.agent.set_transaction_name(self.transaction_name or "VR_Session_Based_Full")
        ...
        model_input = self.preprocess_input(
            model_input,
            self.model_config["combined_feature_configs"],
