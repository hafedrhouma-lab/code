...
#print(feature_store_relative_path)
...
from projects.vendor_ranking.session_based_ranker.utils.bq_client import (
    upload_to_gcs,
...
def read_data_fs(start_date, end_date, country_code):
    filename = 'raw_session_data_fs_' + country_code + '_' + start_date + '_' + end_date + '.parquet'
    ...
    if os.path.exists(filename):
        ...
        data_df = store.get_historical_features(
            ...
            features=[
                ...
                "orders_session_clicks_t5_v1_fv:session_clicks",
        ...
        data_df['session_clicks'] = data_df['session_clicks'].fillna('no_session_clicks')
        ...
        # TODO: should these be filtered?
        # data_df = data_df[data_df["session_clicks"] != "no_session_clicks"]
...
def read_data_gcs_fs(start_date, end_date, country_code, data_points):
    filename = 'raw_session_train_data_fs_' + country_code + '_' + start_date + '_' + end_date + '.parquet'
...
def read_test_data_fs(start_date, end_date, country_code, data_points, is_val=False):
    if is_val:
        filename = 'raw_session_val_data_fs_' + country_code + '_' + start_date + '_' + end_date + '.parquet'
        data_str = 'val'
    else:
        filename = 'raw_session_test_data_fs_' + country_code + '_' + start_date + '_' + end_date + '.parquet'
        data_str = 'test'
...
def read_test_data_gcs_fs(start_date, end_date, country_code, data_points, is_val=False):
    if is_val:
        ...
        data_str = 'test'
    filename = f"raw_{data_str}_session_data_fs_" + country_code + '_' + start_date + '_' + end_date + '.parquet'
    blob = f"{DESTINATION_BLOB_NAME}/{country_code}/"
...
def read_data_inference_fs(start_date, end_date, country_code):
    # filename = 'raw_session_inference_data_fs_' + country_code + '_' + end_date + '.parquet'
    #
    # if os.path.exists(filename):
    #     data_df = pd.read_parquet(filename)
    # else:
...
def read_data_active_inference_fs(start_date, end_date, country_code):
    # filename = 'raw_active_session_inference_data_fs_' + country_code + '_' + end_date + '.parquet'
    #
    # if os.path.exists(filename):
    #     data_df = pd.read_parquet(filename)
    # else:
