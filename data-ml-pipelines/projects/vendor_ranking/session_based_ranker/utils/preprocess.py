...
def is_on_device(data, device):
    if isinstance(data, torch.Tensor):  # If it's a tensor, check its device
        return data.device == device
    elif isinstance(data, list):  # If it's a list, recursively check each element
        return all(is_on_device(item, device) for item in data)
    elif isinstance(data, dict):  # If it's a dictionary, recursively check each value
        return all(is_on_device(value, device) for value in data.values())
    else:  # If it's neither, consider it as on device (e.g., numbers, strings, etc.)
        return True


### Maybe modify this function later to take device as input 
def data_collator_refactored(features, feature_configs):
    ...
    collated_features['labels'] = torch.tensor([f['labels'] for f in features])
...
### Maybe modify this function later to take device as input 
def preprocess_function(examples, feature_configs, area_id_to_index, geohash_to_index, numerical_feat_names, label_tokenizer):
    processed_features = {}
...
def preprocess_test_input(model_input, feature_configs, device):
    tokenized_inputs = {}
    for feature_config in feature_configs:
        name = feature_config['name']
        tokenizer = feature_config['tokenizer']
        ...
        inputs = tokenizer(model_input[name], max_length=max_length, truncation=True, padding="max_length",
                           return_tensors="pt")
        # model_input[f'{name}_ids'] = inputs['input_ids']
        # model_input[f'{name}_mask'] = inputs['attention_mask']

        tokenized_inputs[f'{name}_ids'] = inputs['input_ids'].to(device)
        tokenized_inputs[f'{name}_mask'] = inputs['attention_mask'].to(device)
