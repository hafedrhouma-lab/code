   ...
   WITH
        ...
        orders AS (
            ...
            ON
                ...
                AND dp_foit.order_date  > CURRENT_DATE() - {duration}
                AND dp_foit.order_date <> CURRENT_DATE()
                AND dp_foin.is_successful
                LEFT JOIN `tlb-data-prod.data_platform.dim_item` dp_di
            ON dp_di.item_id = dp_foit.item_id
            WHERE
                dp_foin.order_date <> CURRENT_DATE()
                AND dp_foin.order_date  > CURRENT_DATE() - {duration}
                AND dp_foin.is_successful
