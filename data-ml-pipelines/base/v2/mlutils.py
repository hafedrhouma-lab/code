...
from .talabat_pyfunc import talabat_load_model
    ...
    if use_remote_code:
        ...
        try:
            ...
            if os.path.exists(code_dir):
                ...
                paths_to_add.extend([code_dir, local_path])
                LOG.debug(f"Temporarily adding remote code to sys.path: {local_path}")
                ...
                with temporarily_modify_sys_path(paths_to_add=paths_to_add):
                    ...
                    model = talabat_load_model(model_uri=local_path)
            else:
        ...
        # Use local code
        model = talabat_load_model(model_uri=model_uri)
...
def load_experiment_model(run_id):
    ...
    model = talabat_load_model(model_uri)
...
def register_and_assign_alias(model_uri, model_name, challenger_model_alias):
    try:
        registered_model = mlflow.register_model(model_uri, model_name)
        LOG.info(f"Model registered with URI: {model_uri} and name: {model_name}")
    ...
    try:
        ...
        LOG.info(
            f"Alias '{challenger_model_alias}' "
            f"successfully assigned to model version {model_version}."
        )
    except Exception as e:
        LOG.error(f"Error assigning alias: {e}")
        raise
