...
class BigQuery:
    def __init__(self, json_path=None, project=None, use_query_cache=True):
        ...
        Args:
            credentials (:obj:`int`): path to google service account credentials json file
        """
    ...
    def read(
        ...
        Args:
            ...
            create_bqstorage_client (:bool:): enables BQ Storage API which is a faster way to fetch rows from BQ
            (requires fastavro or pyarrow)
            optimize_download (:bool:): apply optimized way for downloading and converting downloaded data to pd
        Returns:
    ...
    def write(self, df, table_name, if_exist_action, schema=[]):
        ...
        Args:
            df (:obj:`dataframe`): dataframe to be published
            dest_table (:obj:`str`): table name in the form of "dataset.tablename"
            if_exist_action (:obj:`str`): action to take the table already exist.

                - ``'fail'`` If table exists raise pandas_gbq.gbq.TableCreationError.
                - ``'replace'`` If table exists, drop it, recreate it, and insert data.
                - ``'append'`` If table exists, insert data. Create if does not exist.
            schema (:obj:`list`): list of columns type. (only needed for table creation)
