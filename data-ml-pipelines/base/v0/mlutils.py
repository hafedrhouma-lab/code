...
def load_registered_model(model_name, alias, use_remote_code=True):
    ...
    # Get information about the model version
    model_version = client.get_model_version_by_alias(model_name, alias)
    model_tags = model_version.tags
    LOG.info(f"Model Tags: {model_tags}")

    # Print additional model details
    LOG.info(f"Model Version: {model_version.version}")
    LOG.info(f"Current Stage: {model_version.current_stage}")

    # Get run info
    run_id = model_version.run_id
    run_info = client.get_run(run_id)
    LOG.info(f"Run ID: {run_id}")
    LOG.info(f"Run Status: {run_info.info.status}")
    LOG.info(f"Artifact URI: {run_info.info.artifact_uri}")

    # Access and print experiment details
    ...
    LOG.info(f"Experiment Artifact Location: {experiment.artifact_location}")
    ...
    if use_remote_code:
        ...
        try:
            ...
            if os.path.exists(code_dir):
                ...
                LOG.debug(f"Temporarily adding remote code to sys.path: {local_path}, {code_dir}")
...
def register_model(model_name, run, alias):
    # allowed_aliases = ['Champion', 'Development', 'Production']

    """
    Assign an alias to the latest version of a registered model within a specified stage.
    :param alias: The alias to assign to the model version.
    :param run_id: The run ID of the model version.
    :return: None
    """
