...
- requests_filepath : str
    ...
    - e.g., {"prompt": "text-embedding-ada-002", "input": "embed me", "metadata": {"row_id": 1}}
...
- save_filepath : str, optional
    ...
    - e.g., [{"prompt": "text-embedding-ada-002", "input": "embed me"}, {...}]
...
class APIRequest:
    ...
    async def call_api(
        ...
        error = None
        try:
            # disabling certificate verification is not secure and should only be used for debugging purposes
            # to identify the root cause of the SSL verification failure
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    url=request_url, headers=request_header, json=self.request_json, ssl=ssl_context
                ) as response:
    ...
    if api_endpoint.endswith("completions"):
        ...
        # normal completions
            ...
            if isinstance(prompt, str):  # single prompt
                ...
                return num_tokens
            elif isinstance(prompt, list):  # multiple prompt
...
n_requests = 10_000
jobs = [{"prompt": "text-embedding-ada-002", "input": str(x) + "\n"} for x in range(n_requests)]
